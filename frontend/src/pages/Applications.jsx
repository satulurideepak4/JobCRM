  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#00000088', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px', width: '400px' }}>
        <h3 style={{ color: '#e2e8f0', marginBottom: '20px' }}>Add Application</h3>
...
function TimelineRow({ companyId }) {
  ...
  if (!data) return <div style={{ padding: '12px 24px', color: '#64748b', fontSize: '13px' }}>Loading...</div>
  return (
    <div style={{ padding: '12px 24px', borderTop: '1px solid #1e293b' }}>
      {data.length === 0 ? (
        <div style={{ fontSize: '13px', color: '#64748b' }}>No timeline events</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {data.map((event, i) => (
            <div key={i} style={{ display: 'flex', gap: '12px', fontSize: '12px' }}>
              <span style={{ color: '#475569', whiteSpace: 'nowrap' }}>{event.at ? new Date(event.at).toLocaleDateString() : '—'}</span>
...
export default function Applications() {
  ...
  // Scroll to and highlight the company coming from the dashboard
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '700', color: '#e2e8f0' }}>Applications</h1>
        <button
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        <input
          ...
          style={{ flex: 1, padding: '8px 12px', background: '#1e293b', border: '1px solid #334155', borderRadius: '8px', color: '#e2e8f0', fontSize: '14px' }}
        />
        <select
          value={filterStatus}
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #334155' }}>
                {['', 'Company', 'Status', 'Source', 'Added', 'Days Since Update', ''].map((h, i) => (
                  <th key={i} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', color: '#64748b', fontWeight: '500' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={7} style={{ padding: '32px', textAlign: 'center', color: '#475569', fontSize: '14px' }}>No applications yet</td></tr>
              )}
              {filtered.map(c => (
                <>
                  <tr
                  ...
                  >
                    <td style={{ padding: '12px 16px', width: '24px' }}>
                      {expanded === c.id ? <ChevronDown size={14} color="#94a3b8" /> : <ChevronRight size={14} color="#64748b" />}
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ fontSize: '14px', color: '#e2e8f0', fontWeight: '500' }}>{c.name}</div>
                      {c.website && <div style={{ fontSize: '12px', color: '#64748b' }}>{c.website}</div>}
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                      <select
                        value={c.status}
                        ...
                        style={{ background: 'transparent', border: 'none', color: STATUS_COLORS[c.status] || '#94a3b8', fontSize: '12px', cursor: 'pointer' }}
                      >
                        {ALL_STATUSES.map(s => <option key={s} value={s} style={{ background: '#1e293b', color: '#e2e8f0' }}>{s.replace(/_/g, ' ')}</option>)}
                      </select>
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: '12px', color: '#64748b' }}>{c.source?.replace(/_/g, ' ')}</td>
                    <td style={{ padding: '12px 16px', fontSize: '12px', color: '#64748b' }}>{c.added_at ? new Date(c.added_at).toLocaleDateString() : '—'}</td>
                    <td style={{ padding: '12px 16px', fontSize: '12px', color: '#64748b' }}>{daysSince(c.updated_at)}d</td>
